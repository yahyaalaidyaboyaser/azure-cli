...
# - Microsoft Azure CLI (x86)
# - Microsoft Azure CLI (x64)
...
# Show installation log. Since the log is too big, we only print it on demand.
...
# We can't restart the current shell, so use absolute path.
# If we can find a way to refresh the PATH env var, we can directly call az.
